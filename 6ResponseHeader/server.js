...
const requestListener = (request,response) => {
    ...
    if(method === "GET") {
        ...
        response.end(`{"message": "Halaman ${url} ditemukan"}`); 
    } else {
        ...
        response.end(`{'message':'Halaman ${url} tidak dapat diakses menggunakan ${method} ini'}`)
            ...
            if(method === 'GET') {
                    ...
                    request.on('end', () => {
                        ...
                        response.statusCode = 200;
                        response.setHeader('Content-type','application/json');
                        response.end(`{'message': '${name} Ini Halaman About menggunakan method ${method}'}`);
                    })

            } else {
                response.statusCode = 400;
                response.setHeader('Content-type','application/json');
                response.end(`{'message': 'Tidak dapat diakses menggunakan method ${method}'}`);
            }
